...
TEST_CASE("EventBus: subscribe and publish invokes callback", "[game][event]")
{
    ...
    bus.subscribe<BlockPlacedEvent>(EventType::BlockPlaced, [&called](const BlockPlacedEvent&) { called = true; });
...
TEST_CASE("EventBus: multiple subscribers all invoked", "[game][event]")
{
    ...
    bus.subscribe<BlockBrokenEvent>(EventType::BlockBroken, [&callCount](const BlockBrokenEvent&) { callCount++; });
    bus.subscribe<BlockBrokenEvent>(EventType::BlockBroken, [&callCount](const BlockBrokenEvent&) { callCount++; });
    bus.subscribe<BlockBrokenEvent>(EventType::BlockBroken, [&callCount](const BlockBrokenEvent&) { callCount++; });
...
TEST_CASE("EventBus: unsubscribe prevents future callbacks", "[game][event]")
{
    ...
    auto id = bus.subscribe<ChunkLoadedEvent>(
        EventType::ChunkLoaded, [&callCount](const ChunkLoadedEvent&) { callCount++; });
...
TEST_CASE("EventBus: event data passed correctly to subscriber", "[game][event]")
{
    ...
    bus.subscribe<BlockPlacedEvent>(EventType::BlockPlaced, [&](const BlockPlacedEvent& e) {
...
TEST_CASE("EventBus: subscriberCount reflects state", "[game][event]")
{
    ...
    auto id1 =
        bus.subscribe<BlockPlacedEvent>(EventType::BlockPlaced, [](const BlockPlacedEvent&) {});
...
TEST_CASE("EventBus: unsubscribe with invalid id is a no-op", "[game][event]")
{
    ...
    bus.subscribe<BlockPlacedEvent>(EventType::BlockPlaced, [&callCount](const BlockPlacedEvent&) { callCount++; });
